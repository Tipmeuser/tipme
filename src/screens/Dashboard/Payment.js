...
const Payment = () => {
  ...
  const [clientSecret, setClientSecret] = useState(state?.payment?.paymentIntent);
  ...
  useEffect(() => {
    ...
    const loadStripeClient = async () => {
      const stripe = await loadStripe('pk_test_51NdHL7D8sIpXeskQeT6CpplrqrGVFHb5npUb6p44xjXwavDqBNefgfC9VW4VqTKnFJ7bB5bK0OJOwXFfKBu6O9e4006pkHpPmK');
